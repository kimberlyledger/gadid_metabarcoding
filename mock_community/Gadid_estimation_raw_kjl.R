...
#setwd("/home/kimberly.ledger/nwfsc-cb-Toward_Quantitative_Metabarcoding-fba0076/") 

gadid_dat <- read.csv("/genetics/edna/workdir/gadids/20230111/gadid_mock_community_data_20220111.csv") %>%
  select(!X) %>%
...
sp.by.comm <- gadid_dat %>% filter(Cycles==35) %>% 
  group_by(primer_set, species=ID_gadid,community) %>%
  summarize(conc = sum(start_conc_ng)) %>%
  mutate(conc2 = ifelse(conc>0,1,0)) %>%
  arrange(primer_set,species,community) %>% group_by(primer_set, community) %>%
...
### i am going to split up the primer sets here... 
  ...
  env <- gadid_dat %>%  
            ...
            filter(Cycles==35) %>% 
            filter(community %in% c("N2", "S2", "M1")) %>% 
            rename("Species" = "ID_gadid") %>% 
  ...
  #assign reference species    - had issue with this so setting manually instead of using most common
  #mostCommon <- "Boreogadus saida"
  #env$Species[env$Species == mostCommon] <- paste0("zRefSpecies_", mostCommon) #make the most common species the reference species
  #env <- env %>% 
  #  arrange(Species, community)
  ...
  mc <- gadid_dat %>%  
    ...
    filter(Cycles==35) %>% 
    filter(community %in% c("E", "N1", "S1")) %>% 
    rename("Species" = "ID_gadid")  #mock comm samples
  ...
  # Make a single species list:
    ...
    sample_data_small <- sample_data %>% filter(tech_rep== "2_H")
    ...
    mock_data_small <- mock_data %>% filter(tech_rep== "2_H")
...
Output <- list(
  gadid_dat = gadid_dat, # raw data from all observations, all communities.
...
#save(Output,file=paste0("/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S1_raw",".Rdata"))
#save(Output,file=paste0("/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S2_raw",".Rdata"))
#save(Output,file=paste0("/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S3_raw",".Rdata"))
#save(Output,file=paste0("/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S4_raw",".Rdata"))
save(Output,file=paste0("/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S5_raw",".Rdata"))
...
(p9 <- env %>%
    ...
    geom_point() +
    geom_segment(aes(x = Species, xend = Species, y = `25%`, yend = `75%`), linewidth = .6) +
    geom_segment(aes(x = Species, xend = Species, y = `2.5%`, yend = `97.5%`), linewidth = .1) +
    #facet_grid(BiolComm~Cycles, drop = T)  +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ylab("Proportion DNA") +
    guides(color = "none") +
    ggtitle("Gadid S5 (mock community)"))

p9

#ggsave(p9, filename = "/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S1_raw.pdf", width = 7, height = 5, units = "in")
#ggsave(p9, filename = "/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S2_raw.pdf", width = 7, height = 5, units = "in")
#ggsave(p9, filename = "/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S3_raw.pdf", width = 7, height = 5, units = "in")
#ggsave(p9, filename = "/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S4_raw.pdf", width = 7, height = 5, units = "in")
ggsave(p9, filename = "/home/kimberly.ledger/eDNA_metabarcoding/gadid_20220111/gadid_S5_raw.pdf", width = 7, height = 5, units = "in")

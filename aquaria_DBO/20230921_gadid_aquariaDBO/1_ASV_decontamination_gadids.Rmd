...
```{r}
asv_table <- read.csv("/genetics/edna/workdir/gadids/20230921_aquaria/trimmed/filtered/outputs/ASVtable.csv") %>%
...
asv_table_with_sample_type %>%
  pivot_longer(cols = c(3:381), names_to = "ASV", values_to = "reads") %>%
...
replicates_removed_2 %>%
  pivot_longer(cols = c(3:353), names_to = "ASV", values_to = "count") %>%
...
samples_removed_2 <- replicates_removed_2 %>%
  filter(sample_type == "sample") %>%
  select(Sample_ID)
#write.csv(samples_removed_2, "~/gadid_metabarcoding/20230921_gadid_aquariaDBO/20230921_decontamination_step3.csv")
```
...
note: for DBO samples, i will treat a station_ID as a 'site' 
```{r}
sites <- metadata %>%
  filter(MiSeq_run == "B") %>%
  dplyr::select(Sample_ID, extraction_ID, pcr_replicate, alt_ID, tank_ID, station_ID)

sites$combined_column <- ifelse(is.na(sites$tank_ID), sites$station_ID, sites$tank_ID)

rep_table <- asv_table_filter3 %>%
  left_join(sites, by = "Sample_ID") %>%
  filter(combined_column != 'NA') %>%            ##remove sample for which we don't have site locations
  arrange(combined_column)

occu_df <- rep_table %>%
  ungroup() %>%  
  mutate(reads = ifelse(reads > 0, 1, 0)) %>% # change counts to presence/absence
  dplyr::select(ASV, combined_column, alt_ID, Sample_ID, reads) %>%
  group_by(ASV, combined_column, alt_ID) %>%
...
pattern.of.replication <- rep_table %>%
 ...
 # filter(!grepl("-2-", Sample_ID)) %>%    ## the extraction replicates are messing thing up right now... i need to code this better in the metadata/etc eventually 
  ...
  dplyr::select(combined_column, alt_ID, pcr_replicate, ASV, reads) %>%
  pivot_wider(names_from = pcr_replicate, values_from = reads) %>%
  mutate(ndetections = A + B + C) %>%
  group_by(combined_column, ndetections, ASV) %>%
...
temp.df <- occu_df_subset
  ...
  #write_rds(myHierarchicalStanResults, "~/gadid_metabarcoding/20230921_gadid_aquariaDBO/20230921_MiSeqrun_occupancy_output_20230925.rds")
```


```{r}
myHierarchicalStanResults <- read_rds("~/gadid_metabarcoding/20230921_gadid_aquariaDBO/20230921_MiSeqrun_occupancy_output_20230925.rds")
...
keepers <- site.asv_occupancy_probs %>%
  filter(max_Occupancy_prob >= 0.81) %>%
  unite(combined_column, ASV, col = 'filter_id', sep = '.', remove = F)
...
asv_table_filter4 <- rep_table %>% 
  unite(combined_column, ASV, col = 'loc.asv', sep = '.', remove = F) %>%
...
```{r}
remove_5 <- c("e02528-A", "e02565-C", "e02593-A", "e02583-C", "e02538-B")
...
okay so some of the DBO samples have fairly dissimilar pcr replicates... for now i will keep everything. but maybe want to incorporate this sort of thing into another stage of the analysis 
...
```{r}
write.csv(asv_table_filter4, "~/gadid_metabarcoding/20230921_gadid_aquariaDBO/20230921_decontamination_output_20230925.csv", row.names = FALSE)
```

...
analysis of gadid sequences from Feb 21 2023 MiSeq run 
...
```{r}
taxon_S1 <- read.csv("/genetics/edna/workdir/gadids/20230221/S1_ND1/trimmed/filtered/outputs/asv_taxonomy_blastn.csv", row.names = 1) 
taxon_S3 <- read.csv("/genetics/edna/workdir/gadids/20230221/S3_ND5/trimmed/filtered/outputs/asv_taxonomy_blastn.csv", row.names = 1) 

asv_table_S1 <- read.csv("/genetics/edna/workdir/gadids/20230221/S1_ND1/trimmed/filtered/outputs/ASVtable.csv") %>%
  rename(SampleID = X)
asv_table_S3 <- read.csv("/genetics/edna/workdir/gadids/20230221/S3_ND5/trimmed/filtered/outputs/ASVtable.csv") %>%
  rename(SampleID = X)
```
...
alright so it seems like all the eggs were walleye pollock.
...
if there are < around 1000 reads for a sample, the mock community proportions noticeably deviate from what is expected. 
...
greater sequencing depth would likely help with this. 
...
okay, the odd thing here is that i mainly/only found walleye pollock in the NBS samples... and this is according to both primer sets... 
...
combine 12S NBS and gadid specific NBS sequences for comparison
```{r}
NBS_ND <- NBS %>%
  select(!SampleID) %>%
  select(!Project) %>%
